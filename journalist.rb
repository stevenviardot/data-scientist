...
handles_nombre = handles.select {|item| item =~ /[0-9]/}
puts handles_nombre.count

# --------------------------------------------------------------------------------------------------------------------------------
# Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans faire attention à la casse) ?
# --------------------------------------------------------------------------------------------------------------------------------

aude = handles.select { |a| a.downcase.include?("aude") }
puts aude.count
...
handles_start_maj = handles.select {|item| item[1..1] =~ /[A-Z]/}
puts handles_start_maj.count

# --------------------------------------------------------------------------------------------------------------------------------
# Combien contiennent une majuscule ?
# --------------------------------------------------------------------------------------------------------------------------------

handles_maj = handles.select {|item| item =~ /[A-Z]/}
puts handles_maj.count

# --------------------------------------------------------------------------------------------------------------------------------
# Combien de _ dans tous les pseudos confondus ?
# --------------------------------------------------------------------------------------------------------------------------------

dash = handles.select { |a| a.include?("_") }
puts dash.count

# --------------------------------------------------------------------------------------------------------------------------------
# Trie la liste de handle par ordre alphabétique
# --------------------------------------------------------------------------------------------------------------------------------
new_list = handles.map(&:downcase)
result_downcase = new_list.sort
puts result_downcase
